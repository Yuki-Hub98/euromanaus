...
const ProdutoRegister = (props) => {
  const { dataRenderModal, handleValue, dataProduto, AddItem, duplicated, requestArvore, dataArvore, type} = props
  ...
  const modelo = ["teste", "LAZIO 220", "ETTORE 210", "SOFIA"]
              ...
              <div className="w-full h-full grid grid-cols-6 gap-y-2">
                ...
                {duplicated?.departamento ?
                  <Input className="w-full" size="sm" isDisabled name="departamento" value={duplicated?.departamento} labelPlacement="outside"/>
                    :
                  <Select className="w-full" size="sm" se aria-label="departamento" onChange={(e) => {handleValue(e), requestArvore(e)}} name="departamento" labelPlacement="outside">
                ...
                <div className="col-span-2" aria-labelledby="selectFornecedor">
                  <Select className="w-full" size="sm"  onChange={(e) => {handleValue(e)}} aria-label="fornecedor" name="fornecedor" labelPlacement="outside">
                ...
                {duplicated?.linha ?
                  <Input className="w-full" size="sm" isDisabled name="linha" value={duplicated?.linha} labelPlacement="outside"/>
                    :
                  <Select className="w-full" aria-label="linha" size="sm" onChange={(e) => {handleValue(e), requestArvore(e)}} name="linha" labelPlacement="outside">
                ...
                {duplicated?.modelo ?
                  <Input className="w-full" size="sm" isDisabled name="modelo" value={duplicated?.modelo} labelPlacement="outside"/>
                    :
                    <Select className="w-full" aria-label="modelo" size="sm" onChange={(e) => {handleValue(e), requestArvore(e)}} name="modelo" labelPlacement="outside">
                    {modelo?.map((mode) => (
                ...
                {duplicated?.familia ?
                  <Input className="w-full" size="sm" isDisabled name="familia" value={duplicated?.familia} labelPlacement="outside"/>
                    :
                  <Select className="w-full" aria-label="familia" size="sm" onChange={(e) => {handleValue(e), requestArvore(e)}} name="familia" labelPlacement="outside">
        ...
        return(
          <>
            <div className="w-full h-full grid grid-cols-6 gap-y-2">
              ...
              <div className="col-span-2" aria-labelledby="selectDepartamento">
                <Select className="w-full" size="sm" se aria-label="departamento" onChange={(e) => {handleValue(e), requestArvore(e)}} selectedKeys={[dataProduto?.departamento]} name="departamento" labelPlacement="outside">
                  {dataRenderModal?.departamento[0]?.map((departamento) => (
                    <SelectItem key={departamento?.descricao} value={departamento?.descricao}>{departamento?.descricao}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="col-span-2" aria-labelledby="selectFornecedor">
                <Select className="w-full" size="sm"  onChange={(e) => {handleValue(e)}} selectedKeys={[dataProduto?.fornecedor]} aria-label="fornecedor" name="fornecedor" labelPlacement="outside">
                  {dataRenderModal?.fornecedor[0]?.map((fornecedor) => (
                    <SelectItem key={fornecedor.nomeFantasia} value={fornecedor.nomeFantasia}>{fornecedor.nomeFantasia}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="col-span-2" aria-labelledby="selectModel">
                  <Select className="w-full" aria-label="modelo" size="sm" onChange={(e) => {handleValue(e) }} selectedKeys={[dataProduto?.modelo]} name="modelo" labelPlacement="outside">
                  {modelo?.map((mode) => (
                    <SelectItem key={mode} value={mode}>{mode}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="col-span-2" aria-labelledby="selectTipoProduto">
                <Select className="w-full" size="sm" aria-label="tipoProduto" onChange={(e) => {handleValue(e)}} selectedKeys={[dataProduto?.tipoProduto]} name="tipoProduto" labelPlacement="outside">
                  {tipoProduto?.map((tipo) => (
                    <SelectItem key={tipo.status} value={tipo.status}>{tipo.status}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="col-span-2" aria-labelledby="selectGrupo">
                <Select className="w-full" size="sm" name="grupo" onChange={(e) => {handleValue(e), requestArvore(e)}} aria-label="grupo" labelPlacement="outside">
                  {dataArvore?.grupo[0]?.map((grupo) => (
                    <SelectItem key={grupo.grupo} value={grupo.grupo}>{grupo.grupo}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="col-span-2" aria-labelledby="selectUnidade">
                <Select className="w-full" size="sm" name="unidadeMedida" onChange={(e) => {handleValue(e)}} selectedKeys={[dataProduto?.unidadeMedida]} aria-label="unidadeMedida" labelPlacement="outside">
                  {unidadeMedida?.map((unidade)=> (
                    <SelectItem key={unidade.medida} value={unidade.medida}>{unidade.medida}</SelectItem>
                  ))}
                </Select>
              ...
              <div className="h-24 mt-6 pt-4 col-start-2 col-end-6 grid grid-cols-6 rounded-md items-center border shadow">
                ...
                <div className="col-span-2 pr-2" aria-labelledby="selectEspecificacao">
                  <Select className="w-full" size="sm" name="especificacao" onChange={(e) => {handleValue(e)}} aria-label="especificacao" 
                ...
                {isDisabled && valueTable?  
                  ...
                  : 
                    <div className="col-end-7 justify-self-end pr-2" aria-labelledby="adicionar">
                      <Button size="sm" name="adicionar" className="bg-[#edca62b4] shadow-lg shadow-indigo-500/20" isDisabled>
                        Editar
                      </Button>
                    </div>
                  }
