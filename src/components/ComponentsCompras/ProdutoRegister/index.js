...
const ProdutoRegister = (props) => {
  const { dataRenderModal, handleValue, dataProduto, AddItem, duplicated} = props
    ...
    <div className="w-full h-full grid grid-cols-6 gap-y-2">
      ...
      {duplicated?.modelo ?
        <Input className="w-full" size="sm" isDisabled name="modelo" value={duplicated?.modelo} labelPlacement="outside"/>
          :
          <Select className="w-full" aria-label="modelo" size="sm" onChange={(e) => {handleValue(e), RequestLinha(e)}} name="modelo" labelPlacement="outside">
          {modelo?.map((mode) => (
