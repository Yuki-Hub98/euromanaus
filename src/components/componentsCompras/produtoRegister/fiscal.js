...
const Fiscal = (props) => {
  const {dataFiscal, handleValue, SetData, duplicated} = props
  ...
  <>
    <div className="w-full h-full grid grid-cols-4 gap-y-1 items-center">
      <div  aria-labelledby="origem">
        <span className="text-xs items-center">ORIGEM</span>
      </div>
      <div className="col-span-3" aria-labelledby="selectOrigem">
        {duplicated?.cstIcmsOrigem ? 
          <Input className="w-3/5 text-xs" isDisabled value={duplicated?.cstIcmsOrigem} labelPlacement="outside" aria-label="origem" size="sm" name="cstIcmsOrigem"/>
            :
          <Select className="w-3/5 text-xs" labelPlacement="outside" aria-label="origem" placeholder={`${dataFiscal?.cstIcmsOrigem ? dataFiscal?.cstIcmsOrigem: '' }`} size="sm" name="cstIcmsOrigem" onChange={(e) => {handleValue(e), requestCstIcms(e)}}>
      ...
      <div  aria-labelledby="cstICMS">
        <span className="text-xs items-center">CST ICMS</span>
      </div>
      <div className="col-span-3" aria-labelledby="selectCstIcmsDescricao">
        { duplicated?.cstIcmsDescricao ?
          <Input className="w-3/5 text-xs" isDisabled value={duplicated?.cstIcmsDescricao} labelPlacement="outside" aria-label="origem" size="sm" name="cstIcmsOrigem"/>
            :
          <Select className="w-full" labelPlacement="outside" aria-label="csticms" placeholder={`${dataFiscal?.cstIcmsDescricao ? dataFiscal?.cstIcmsDescricao : ''}`} name="cstIcmsDescricao" onChange={(e) => {handleValue(e)}} size="sm">
      ...
      <div  aria-labelledby="cstPisConfins">
        <span className="text-xs items-center">CST PIS/Cofins</span>
      </div>
      <div className="col-span-3" aria-labelledby="selectCstPisConfins">
        {duplicated?.cstPisConfins ? 
          <Input className="w-3/5 text-xs" isDisabled value={duplicated?.cstPisConfins} labelPlacement="outside" aria-label="origem" size="sm" name="cstIcmsOrigem"/>
        :
          <Select className="w-full" aria-label="cstPisConfins" name="cstPisConfins" placeholder={`${dataFiscal?.cstPisConfins ? dataFiscal?.cstPisConfins : ''}`} onChange={(e) => handleValue(e)} labelPlacement="outside" size="sm">
      ...
      <div  aria-labelledby="ncm">
        <span className="text-xs items-center">NCM</span>
      </div>
      <div >
        {duplicated?.ncmCodigo ? 
          <Input className="w-full" isDisabled value={duplicated?.ncmCodigo} labelPlacement="outside" aria-label="origem" size="sm" name="ncmCodigo"/>
            :
      ...
      <div className="flex items-center col-span-2  w-40" aria-labelledby="porcetagemForaEstado">
        <span className="text-xs w-28 items-center">% Fora do Estado</span>
        <Input className="w-5/12" name="porcetagemForaEstado" labelPlacement="outside" size="sm" />
      </div>
      <div  aria-labelledby="substTributaria">
        <span className="text-xs items-center">Subst Tributária</span>
      </div>
